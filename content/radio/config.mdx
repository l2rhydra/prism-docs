...
Config.DefaultSettings = {
    ...
    -- Default saved nickname (empty string = no saved nickname)
    savedNickname = '',
}
...
-- 2 = disabled, use this if you have your own radiolist
Config.RadiolistStyle = 1

-- Should the radiolist be always expanded? (= show all members of channel)
-- Warning: if a radio channel is going to have 15-20 members, it might get annoying to be on screen.
-- ONLY applies to the advanced (1) radiolist style, as the minimalistic (0) style is always expanded and the disabled (2) style doesn't have a radiolist.
Config.RadiolistExpanded = false

-- Which jobs should not see the radiolist.
Config.RadiolistJobBlacklist = {
    -- ["police"] = true
}
...
Config.ItemName = {
    "radio"
} -- Item name if RequireItem is true (you can add multiple)

-- Automatically disconnect player from radio channel if they no longer
-- have any of the required radio items in their inventory
Config.DisconnectOnNoItem = true
...
Config.PropServerSideSpawn = false -- Should props spawn server-side?
...
-- When true, players cannot open the settings page in the radio UI.
Config.DisableSettings = false

-- Automatically disconnect player from radio channel on death?
Config.DisconnectOnDeath = true

-- Allow opening the radio while being dead?
Config.AllowUseWhileDead = false
...
Config.AllowChange = true -- Allow users to change talk animation in settings?
